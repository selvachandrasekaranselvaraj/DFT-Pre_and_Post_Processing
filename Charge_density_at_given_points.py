...
################################################################################
cmd = "ls electronic_density.cube"
status, output = commands.getstatusoutput(cmd)
if "electronic_density.cube" in output.split():
    ...
    status, output = commands.getstatusoutput(cmd)
    if "electronic_density.CUBE" in output.split():
        os.system("cp electronic_density.CUBE electronic_density.cube")
        output                          = Read_charge("electronic_density.cube")
        Xvoxel, Yvoxel, Zvoxel          = output[0], output[1], output[2]
        lattice_a, lattice_b, lattice_c = output[3], output[4], output[5]
        tot_charge, charge              = output[6], output[7]
        ...
        print "electronic_density.CUBE file is NOT in the present directory" 
        exit()
...
cmd = "ls input_coordinates.txt"
status, output = commands.getstatusoutput(cmd)
if "input_coordinates.txt" in output.split():
    print 'input_coordinates.txt is presented in the directory'
    print
else:
    print 'input_coordinates.txt is NOT presented in the directory'
    print
    print "             *****************    "
    print "     Example input_coordinates.txt file "
    print "Cd  9.3600 9.3600 9.3600"
    print "Cd_vac  9.3600 9.3600 9.3600"
    print
    exit()
...
savefig('radial_charges', bbox_inches='tight') 
show()
print
print "Done"
